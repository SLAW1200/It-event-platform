// Server-only API client (uses next/headers cookies) + the response types the
// dashboard pages import. Single ingress to the backend gateway.
...
// Empty object (not an empty Bearer) when there's no session, so no header is sent.
async function authHeader(): Promise<Record<string, string>> {
  const token = (await cookies()).get('eventra_token')?.value
  return token ? { Authorization: `Bearer ${token}` } : {}
}

// Authenticated fetch. no-store because dashboard data is per-user and the Next
// cache would leak it across users; failures come back as a tagged union.
...
// Unauthenticated fetch for the public attendee surface — no cookie/auth header.
...
// Decodes (does NOT verify) the JWT payload to read id/email/role server-side
// for rendering. Safe here because the gateway re-verifies the signature on
// every API call. Returns null if the cookie is missing, malformed, or expired.
...
// Response shapes — one type per endpoint the app reads.
