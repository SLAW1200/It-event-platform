...
// Login/register/logout server actions: validate, call the gateway, and on
// success set the eventra_token httpOnly cookie that api.ts reads. Wired to
// <form action> + useActionState for inline errors.
...
// httpOnly (XSS-resistant), SameSite=Lax so it survives top-level navigations
// from the public event page, 7-day TTL to match the backend JWT expiry.
