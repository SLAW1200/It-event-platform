...
// Server action backing the "new event" form. Validates dates/capacity
// server-side, calls POST /events with organizerId = current user, and
// redirects to /dashboard on success. Returns inline field errors via
// useActionState on failure so the form re-renders with messages preserved.
